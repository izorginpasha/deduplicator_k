...
async def handle_event(event: dict):
    logger.info(f"✅ Уникальное событие: {event['event_id']}")
    # await save_event(event)  # Здесь можно сохранить в БД или отправить в другую систему
...
# ─── Kafka consumer ────────────────────────────────────────────────────────────
async def consume():
    await deduplicator.reset()  # Очистка (только в dev/тестах)
        ...
        async for msg in consumer:
            try:
                ...
                if not await deduplicator.is_duplicate(event):
                    ...
                    logger.info(f"⛔ Дубликат: {event['event_id']}")
            except Exception as e:
